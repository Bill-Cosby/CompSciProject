...
bool Frame::target_position(int row_0, int col_0) {
	...
	// If the target position is watter, wall or snow stop the actor to go through it
	if (target == '~' || target == '#' || target == 'S') {
		return FALSE;
	}
	// If the target position is a monster don't let the actor to go through it (for now)
	if (target == 'M') {
		return FALSE;
	}
	return TRUE;
